...
class ProspectorConfiguration(BaseModel):
    ...
    """
    A list of tools to run. This lets you set exactly which tools to run. To add extra tools to the defaults, see --with-tool.
    """
    with_tools: list[str] = []
    """
    ...
    """
    A list of tools that should not be run. Useful to turn off only a single tool from the defaults. To specify all tools explicitly, use the --tool argument.
    """
    profiles: list[str] = []
    """
...
) -> ProspectorConfiguration:
    return tyro.cli(ProspectorConfiguration)  # , prog==prog, description=description)
